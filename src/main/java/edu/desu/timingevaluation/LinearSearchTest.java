...
import edu.desu.sortsearchutils.LinearSearch;
...
public class LinearSearchTest {
    ...
    public static void TimingTestRun(int n1, int n2, int timesToRun) {
        ...
        if (!"true".equals(System.getProperty("is.student.test.run"))) {
            ...
            int warmUpSize = 500;
            int[] warmUpInput = HelperStaticMethods.generateScrambledIntArray(warmUpSize);
            int[] warmUpTest = HelperStaticMethods.generateScrambledIntArray(warmUpSize);

            for (int i = 0; i < 15000; i++) { // 15,000 is the standard Tier 4 JIT invocation threshold
                // Keep the target dynamic to mimic the benchmark loop pattern
                LinearSearch.linearSearch(warmUpInput, warmUpTest[i % warmUpSize]);
            }
        ...
        System.out.println("Running linearSearch...");
        ...
        for (int i = 0; i < timesToRun; i++) {
            LinearSearch.linearSearch
                    (inputArray, testValues[searchindex]);
        ...
        ourTimingEvaluation.setClassName(LinearSearchTest.class.getSimpleName());
        ...
        System.out.println("Building main and test array...");
        inputArray = HelperStaticMethods.generateScrambledIntArray(inputArraySize2);
